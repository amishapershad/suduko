...
// Helper GPU kernel for solve_boards
__global__ void solveBoard(board_t* boards) {
  // currBoard is shared in the block, keep track of information of current board
  __shared__ uint16_t currBoard[BOARD_DIM * BOARD_DIM];

  // index of the cell current thread is responsible for
  int index = threadIdx.x * 9 + threadIdx.y;

  // copy the data that is shared by all threads to the memory that is only shared in block
  currBoard[index] = boards[blockIdx.x].cells[index];

  // ensures that the thread finishes writing the board before we see examine the board
  __syncthreads();

  // number of cells that already has a value
  int current = __syncthreads_count(cell_to_digit(currBoard[index]));

  // count how many times in a row the board doesn't change
  int count = 0;

  // keep track of the cells that this thread should look at, -1 means it has been examined
  int array[3 * 9];

  // fill array with indices of cells that is related to current cell
  ...
  // region
  for (int s = indX / 3 * 3; s < indX / 3 * 3 + 3; s++) {
    for (int z = indY / 3 * 3; z < indY / 3 * 3 + 3; z++) {
      array[s % 3 * 3 + z % 3 + 18] = s * 9 + z;
      if (s == indX && z == indY) {
        array[s % 3 * 3 + z % 3 + 18] = -1;
      }
    }
  }

  // loop until this cell has a set value, or nothing on the boards has been changed for 2 loops
  while (cell_to_digit(currBoard[index]) == 0 && count < 2) {
    // iterate through array of cells this thread should check
    for (int i = 0; i < 3 * 9; i++) {
      if (array[i] >= 0) {  // not been checked
        int current = cell_to_digit(currBoard[array[i]]);
        if (current != 0) {  // masking
          currBoard[index] = currBoard[index] & (~(digit_to_cell(current)));
          array[i] = -1;
        }
      }
    }

    // if syncthreads_count returns the same number, it means the board has not been modified since
    // last examined.
    if (current == (current = __syncthreads_count(currBoard[index]))) {
      count++;
    } else {
      count = 0;
    }
  ...
  while (__syncthreads_count(1) != 81)
    ;

  // copy memory back to the shared memory address
  boards[blockIdx.x].cells[index] = currBoard[index];
}
...
void solve_boards(board_t* boards, size_t num_boards) {
  ...
  // alloacte memory for gpu to use
  if (cudaMallocManaged(&cudaBoard, BATCH_SIZE * (sizeof(board_t))) != cudaSuccess) {
    fprintf(stderr, "Failed to malloc space for cuda board");
    exit(2);
  }

  // number of threads per block
  dim3 blockThread(BOARD_DIM, BOARD_DIM);

  // copy each board to cudaBoard
  for (int i = 0; i < BATCH_SIZE; i++) {
    cudaMemcpy(&cudaBoard[i], &boards[i], sizeof(board_t), cudaMemcpyHostToDevice);
  }

  // distribute tasks to gpu threads
  solveBoard<<<num_boards, blockThread>>>(cudaBoard);
  ...
  // copies memory back to host memory
  for (int i = 0; i < BATCH_SIZE; i++) {
    cudaMemcpy(&boards[i], &cudaBoard[i], sizeof(board_t), cudaMemcpyDeviceToHost);
  }
}
